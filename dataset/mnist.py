...
# 定义一个函数用于下载所有MNIST数据集文件
def download_mnist():
    for v in key_file.values():
       _download(v)

# 定义一个函数用于加载标签数据并将其转换为NumPy数组
...
# 定义一个函数用于将MNIST数据集转换为NumPy数组并存储在字典中
def _convert_numpy():
    dataset = {}
...
# 定义一个函数用于加载MNIST数据集，并可选地进行归一化、扁平化和one-hot编码
